...
const placeholderScreens = [
  { label: 'Home', icon: '🏠', desc: 'Main dashboard' },
  { label: 'Detail', icon: '📋', desc: 'Detail view' },
  { label: 'Search', icon: '🔍', desc: 'Search & filter' },
  { label: 'Profile', icon: '👤', desc: 'User profile' },
  { label: 'Settings', icon: '⚙️', desc: 'Settings' },
]

export default function PhoneMockup({ screenshotCount, color, platform, projectId }: Props) {
  const [current, setCurrent] = useState(0)
  const count = Math.min(screenshotCount, placeholderScreens.length)
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="relative flex items-center gap-3">
        ...
        >
          ...
          <div
            ...
            style={{ right: -8, top: 60, width: 4, height: 30, background: '#2A2A35' }}
          />
          <div
            className="absolute rounded-l-sm"
            style={{ left: -8, top: 50, width: 4, height: 20, background: '#2A2A35' }}
          />
          <div
            className="absolute rounded-l-sm"
            style={{ left: -8, top: 80, width: 4, height: 30, background: '#2A2A35' }}
          />
        </div>
      ...
      <p className="text-[10px] text-muted font-mono text-center">
        {isAndroid ? 'iOS + Android' : 'iOS only'} · screenshots coming soon
      </p>
    </div>
  )
}
